...
impl RelayServerConfig {
    pub fn from_env() -> Result<Self, ConfigError> {
        ...
        let jwt_secret_str = env::var("KIRACODE_REMOTE_JWT_SECRET")
...
fn validate_jwt_secret(secret: &str) -> Result<(), ConfigError> {
    let decoded = BASE64_STANDARD
        ...
        .map_err(|_| ConfigError::InvalidVar("KIRACODE_REMOTE_JWT_SECRET"))?;

    if decoded.len() < 32 {
        return Err(ConfigError::InvalidVar("KIRACODE_REMOTE_JWT_SECRET"));
    }
