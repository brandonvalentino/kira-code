...
impl ServerHandler for McpServer {
    fn get_info(&self) -> ServerInfo {
        ...
        let preamble = match self.mode() {
            McpMode::Global => {
                "A Kira Code MCP server for task, issue, repository, workspace, and session management."
            }
            McpMode::Orchestrator => {
                "An orchestrator-scoped Kira Code MCP server with tools limited to the configured workspace and orchestrator session context."
            }
        };
