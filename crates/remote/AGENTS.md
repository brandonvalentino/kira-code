...
The `remote` crate is the hosted Kira Code Cloud server: an Axum HTTP API, a React SPA frontend, and real-time sync via ElectricSQL.
...
Kira Code uses [ElectricSQL](https://electric-sql.com) as a read-path sync engine: Postgres → ElectricSQL → clients over HTTP shapes. Writes go through the REST API.
...
- **JWT** (`auth/jwt.rs`): Signed with `KIRACODE_REMOTE_JWT_SECRET`. All protected routes use `require_session` middleware.
