...
import { projectStatuses } from '../db/schema.js';
...
export function projectStatusesRouter(state: AppState) {
  ...
  app.post('/', auth, async (c) => {
    ...
    const [row] = await state.db.insert(projectStatuses).values({ id: data.id ?? uuidv4(), ...data }).returning();
  ...
  app.patch('/:id', auth, async (c) => {
    ...
    const row = await state.db.select().from(projectStatuses).where(eq(projectStatuses.id, id)).limit(1).then(r => r[0]);
    ...
    const [updated] = await state.db.update(projectStatuses).set(data).where(eq(projectStatuses.id, id)).returning();
  ...
  app.delete('/:id', auth, async (c) => {
    ...
    if (!row) return c.json({ error: 'Not found' }, 404);
    if (!(await assertProjectMember(state, user.id, row.project_id))) return c.json({ error: 'Forbidden' }, 403);
    await state.db.delete(projectStatuses).where(eq(projectStatuses.id, id));
