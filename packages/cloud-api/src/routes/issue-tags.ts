...
import { issueTags } from '../db/schema.js';
...
export function issueTagsRouter(state: AppState) {
  ...
  app.post('/', auth, async (c) => {
    ...
    const [row] = await state.db
      .insert(issueTags)
      .values({ id: data.id ?? uuidv4(), issue_id: data.issue_id, tag_id: data.tag_id })
      .returning();
  ...
  app.delete('/:id', auth, async (c) => {
    ...
    const row = await state.db.select().from(issueTags).where(eq(issueTags.id, id)).limit(1).then(r => r[0]);
    if (!row) return c.json({ error: 'Not found' }, 404);
    if (!(await assertIssueMember(state, user.id, row.issue_id))) return c.json({ error: 'Forbidden' }, 403);
    await state.db.delete(issueTags).where(eq(issueTags.id, id));
